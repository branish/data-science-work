...
#df_wine %>% group_by(country) %>% summarize(n = n()) %>% arrange(desc(n))
#df_wine %>% filter(country == "US") <- df_italy$description %>% fnMakeWordCountMatrix()
...
# temp <- df_wine %>% filter(country == 'US') %>% slice(1)
# corp <- Corpus(VectorSource(temp$description))
# toSpace <- content_transformer(function (x , pattern ) gsub(pattern, " ", x))
# corp <- tm_map(corp, toSpace, "[\\.,(\\s-\\s)]")
# corp <- tm_map(corp, toSpace, "\\s-\\s")
# corp <- tm_map(corp, toSpace, "\\\\u0097")
...
# Something online recommended doing sparseMatrix instead, but I still can't turn it back into a matrix if it's too large...
...
```
```{r}
df_scraper <- read_csv('./data/facts_about_winereviews94p.csv', na = 'n/a') %>%
  select(-vintage, -natural, -organic, -biodynamic, -buyUrl, -structureCount, -aromaIntensity, -primaryAromas, -secondaryAromas, -tertiaryAromas, -balance, -length, -finish, -complexity, -conclusion, -ageing, -quality, -body, -noseDevelopment, -noseIntensity, -colorIntensity, -color, -tertiaryFlavors)
glimpse(df_scraper)


jsonSample <- df_scraper$primaryFlavors[1]
library(jsonlite)
df_json <- fromJSON(jsonSample)
df_json %>% tibble()

df_small <- df_scraper %>% slice(1:3)
df_flavors <- map_df(df_small$primaryFlavors, fromJSON) %>% group_by(name) %>% summarize(count = sum(count))
df_small %>% slice(c(1, 3)) %>% mutate(flavorlist = map(primaryFlavors, fromJSON), )
#typeof(fromJSON(df_small$primaryFlavors[1]))
```
