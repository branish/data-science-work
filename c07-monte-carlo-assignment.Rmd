...
n_samples <- 100 # Choose a sample size # NOT USED right now

df_q1 <-
  map_dfr(
    rep(1e5, n_samples),
    function(n) {
      tibble(x = runif(n, min = 0, max = 1), y = runif(n, min = 0, max = 1)) %>%
      summarize(ratio = sum((y <= sqrt(1 - x^2)))/n()) %>%
      mutate(pi = 4 * ratio) # Generate the data
    }
  )
df_q1
